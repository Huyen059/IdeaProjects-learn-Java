...
class MultifunctionalMapper {
     ...
     */
    public static final Function<List<IntUnaryOperator>, UnaryOperator<List<Integer>>> MULTIFUNCTIONAL_MAPPER = intUnaryOperators -> {
        return integers -> {
            return integers.stream().map(integer -> {
                for (IntUnaryOperator intUnaryOperator : intUnaryOperators) {
                    integer = intUnaryOperator.applyAsInt(integer);
                }
                return integer;
    ...
    public static final UnaryOperator<List<Integer>> SQUARE_AND_THEN_GET_NEXT_EVEN_NUMBER_TRANSFORMATION =
            MULTIFUNCTIONAL_MAPPER.apply(Arrays.asList(x -> x * x,
                    x -> x % 2 == 0 ? x + 2 : x + 1));
